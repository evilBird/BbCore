...
- (void)textFieldDidChange:(UITextField *)sender {
    ...
    NSArray *searchResults = [self.dataSource quickType:self suggestedTextForUserText:searchString];
...
- (void)highlightSuggestionButton:(id)sender
{
...
- (void)updateQuickTypeWithContents:(NSArray *)contents frame:(CGRect)frame {
	...
	for (int i = 0; i < contents.count; i++) {
        ...
        [suggestionButton addTarget:self action:@selector(highlightSuggestionButton:) forControlEvents:UIControlEventTouchDown];
